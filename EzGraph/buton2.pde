				...
				if (START==false)
					...
					if (numeUtilizator.s!="" && parola.s!="") {
						...
						while(db.next()){
							...
							if(compareStrings(parola.s, pass) && (compareStrings(numeUtilizator.s, nume) || compareStrings(numeUtilizator.s, email))){
								OKLOGIN = true;
								nmUsr = usrrr;
								currentModule=1;
								EROARELOGARE=false;
						...
						if(!OKLOGIN){
							...
							if(!usrProf){
								...
								menu2.tabs[2].sub[1] = new SubMenu("Test", 13, menu2.tabs[2].seekPos, menu2.tabs[2].seekCol, menu2.tabs[2].seekSize, menu2.tabs[2].animSpd, menu2.tabs[2].seekW, 2*menu2.tabs[2].h/3);
								...
								menu2.tabs[2].sub[3] = new SubMenu("Game Ranking", 19, menu2.tabs[2].seekPos, menu2.tabs[2].seekCol, menu2.tabs[2].seekSize, menu2.tabs[2].animSpd, menu2.tabs[2].seekW, 2*menu2.tabs[2].h/3);
					...
					if (nume.s!="" && prenume.s!="" && numeUtilizator.s!="" && parola.s!="" &&  parola2.s!="" && email.s!=""
						...
						if(!EROAREINREGISTRARE){
							db.execute("INSERT INTO users VALUE (NULL, '"+nume.s+"', '"+prenume.s+"', '"+numeUtilizator.s+"', '"+email.s+"', '"+parola.s+"', "+isProf+", NULL)");
							currentModule=1;
							EROAREINREGISTRARE=false;
					...
					if(nume.s!="" && prenume.s!="" && numeUtilizator.s!="" && parola.s!="" &&  parola2.s!="" && email.s!=""
						...
						if(!EROAREINREGISTRARE){
							db.execute("UPDATE users SET name='"+nume.s+"', surname='"+prenume.s+"', nickname='"+numeUtilizator.s+"', email='"+email.s+"', pass='"+parola.s+"' WHERE userID="+IDUSER+"");
